...
@section('css')
<title>{{$subcat->name}}: Sade Store</title>
...
<meta property=”og:title” content=”{{$subcat->name}}: Sade Store” />
<meta property=”og:description” content=”@foreach ($seods as $seod){{ $loop->first ? '' : ', ' }}{{ $seod->description }}@endforeach” />
@if($subcat->poster != 'default.jpg')
<meta property=”og:image” content=”//img.sade.store/ph/sct/{{$subcat->poster}}” />
@else
<meta property=”og:image” content=”//img.sade.store/catposter.jpg” />
@endif
<meta property=”og:url” content=”//sade.store/subcategory/{{$subcat->slug}}” />
@endsection
@section('body')
      ...
      <div class="row">
        <div class="col-xs-12">
          <ul><li class="home"> <a title="{{ trans('app.go_back_to_home')}}" href="/">{{ trans('app.home')}}</a><span>&raquo;</span></li>
              <li><a href="/category/{{$cat->slug}}" title="{{$cat->name}}">{{$cat->name}}</a><span>&raquo;</span></li>
            <li><strong title="{{$subcat->name}}">{{$subcat->name}}</strong></li></ul>
        </div>
      </div>
    </div>
  </div>
  <div class="main-container col2-left-layout">
    <div class="container">
    <div class="category-description">@if($subcat->poster != 'default.jpg') <a href="/subcategory/{{$subcat->slug}}" title="{{$subcat->name}}"><img src="//img.sade.store/ph/sct/{{$subcat->poster}}" alt="{{$subcat->name}}"></a> @else <a href="/" title="Sade Store"><img src="//img.sade.store/catposter.jpg" alt="Sade Store"></a>@endif</div>
      <div class="row">
        <div class="col-main col-sm-9 col-xs-12 col-sm-push-3">
          <div class="shop-inner">
            <div class="page-title"><h2>{{$subcat->name}}</h2></div>
            <div class="toolbar">
              @if(2==2)
              <div class="sorter">
                <div class="short-by">
                  ...
                  <select onchange="if (this.value) window.location.href=this.value">
                    <option @if($sr == 0) selected @endif value="/subcategory/{{$subcat->slug}}/0">{{__('app.Newest')}}</option>
                    <option @if($sr == 1) selected @endif value="/subcategory/{{$subcat->slug}}/1">A-Z</option>
                    <option @if($sr == 2) selected @endif value="/subcategory/{{$subcat->slug}}/2">Z-A</option>
                    <option @if($sr == 3) selected @endif value="/subcategory/{{$subcat->slug}}/3">{{__('app.High_to_low')}}</option>
                    <option @if($sr == 4) selected @endif value="/subcategory/{{$subcat->slug}}/4">{{__('app.Low_to_high')}}</option>
                  </select>
                </div>
              </div>
              @endif
